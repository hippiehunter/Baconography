    ...
    public:
        virtual void Run(IBackgroundTaskInstance^ taskInstance)
        {
            ...
            auto tileUpdateTask = RunTileUpdater(lockScreenSettings, lockScreenHistory)
                .then([=](task<void> task)
            {
                return ImageUtilities::ClearOldTempImages(concurrency::cancellation_token::none());
            })
                .then([=](task<void> task)
            {
        ...
        task<void> RunPrimaryTileUpdater(LockScreenSettings^ settings, LockScreenHistory^ history, LiveTileSettings^ liveTile)
        {
                ...
                .then([=](task<std::vector<tuple<String^, String^>>> messagesTask)
            {
				try
				{
					...
					return create_task(ImageUtilities::MakeLiveTileImages(vector<ImageInfo^> {}, history, liveTileImageUrls, GetTileCountTarget(true), 0, concurrency::cancellation_token::none()))
